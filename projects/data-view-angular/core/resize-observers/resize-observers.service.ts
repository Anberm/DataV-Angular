...
export class DvResizeObserverFactory {
...
@Injectable({ providedIn: 'root' })
export class DvResizeObserver implements OnDestroy {
  ...
  constructor(private dvResizeObserverFactory: DvResizeObserverFactory) {}
  ...
  private observeElement(element: Element): Subject<ResizeObserverEntry[]> {
    if (!this.observedElements.has(element)) {
      ...
      const observer = this.dvResizeObserverFactory.create(mutations => stream.next(mutations));
      if (observer) {
